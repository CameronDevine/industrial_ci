...
function run_source_tests {
    ...
    TARGET_WORKSPACE="${TARGET_WORKSPACE:-$TARGET_REPO_PATH}" ici_build_workspace "target" "$extend" "$target_ws"
    ...
    if [ "$CATKIN_LINT" == "true" ] || [ "$CATKIN_LINT" == "pedantic" ]; then
        ...
        ici_run "catkin_lint" ici_exec_in_workspace "$extend" "$target_ws"  catkin_lint --explain "${catkin_lint_args[@]}" src
