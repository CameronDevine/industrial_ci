...
* **DOWNSTREAM_WORKSPACE** (default: not set): Definition of downstream `workspace <#workspace-management>`_.
...
* **TARGET_WORKSPACE** (default: ``$TARGET_REPO_PATH``): Definition of sources for target `workspace <#workspace-management>`_.
* **UPSTREAM_WORKSPACE** (default: not set): Definition of upstream `workspace <#workspace-management>`_.
...
A. Upstream workspace: Source packages that are needed for building or testing the target or downstream packages
   ...
   3. Build workspace, chained to /opt/ros
...
B. Target workspace: Packages in your target repository that should get build and tested
   ...
   2. Install dependencies with ``rosdep``
   3. Build workspace, chained to upstream workspace or /opt/ros
   4. run tests (opt-out with ``NOT_TEST_BUILD``)

C. Target workspace: Packages that should get tested against your target repository

   1. Fetch source code (``DOWNSTREAM_WORKSPACE``)
   2. Install dependencies with rosdep
   3. Build workspace, chained to target workspace
   4. run tests (opt-out with ``NOT_TEST_DOWNSTREAM``)

Workspace definition
++++++++++++++++++++

Each workspace can be composed as a sequence of the folling items:
...
* URL of a source repository with the pattern ``<scheme>:<resource>#<version>``, e.g. ``github:ros-industrial/industrial_ci#master``.
  ...
  * ``git``/``git+*``: for any other git repository
  ...
  For convenience ``@`` can be used as a separator as well.
...
* directory path prefixed with ``-`` to remove the directory
